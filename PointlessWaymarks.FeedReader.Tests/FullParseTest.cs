...
public class FullParseTest
{
    ...
    {
        var feed = (AtomFeed)(await Reader.ReadFromFileAsync("Feeds/AtomTheVerge.xml").ConfigureAwait(false))
            .SpecificFeed;
    ...
    {
        var feed = (AtomFeed)(await Reader.ReadFromFileAsync("Feeds/AtomYoutubeInvestmentPunk.xml")
            .ConfigureAwait(false)).SpecificFeed;
    ...
    {
        var feed = (Rss091Feed)(await Reader.ReadFromFileAsync("Feeds/Rss091FullSample.xml").ConfigureAwait(false))
            .SpecificFeed;
        Eq("Copyright 1997-1999 UserLand Software, Inc.", feed.Copyright);
        Eq("Thu, 08 Jul 1999 07:00:00 GMT", feed.PublishingDateString);
        Eq("Thu, 08 Jul 1999 16:20:26 GMT", feed.LastBuildDateString);
    ...
    {
        var feed = (Rss091Feed)(await Reader.ReadFromFileAsync("Feeds/Rss091Stadtfeuerwehr.xml").ConfigureAwait(false))
            .SpecificFeed;
    ...
    {
        var feed = (Rss092Feed)(await Reader.ReadFromFileAsync("Feeds/Rss092FullSample.xml").ConfigureAwait(false))
            .SpecificFeed;
    ...
    {
        var feed = (Rss10Feed)(await Reader.ReadFromFileAsync("Feeds/Rss10FeedWebResourceSample.xml")
            .ConfigureAwait(false)).SpecificFeed;
    ...
    public void TestRss10ParseOrfAt()
    {
        ...
        Eq("Die aktuellsten Nachrichten auf einen Blick - aus Österreich und der ganzen Welt. In Text, Bild und Video.",
            feed.Description);
    ...
    {
        var feed = (Rss10Feed)(await Reader.ReadFromFileAsync("Feeds/Rss10OrfAt.xml").ConfigureAwait(false))
            .SpecificFeed;
        Eq("news.ORF.at", feed.Title);
        Eq("http://orf.at/", feed.Link);
        Eq("2017-01-23T21:54:55+01:00", feed.DC.DateString);
    ...
    public void TestRss20ParseCodeHollow()
    {
        ...
        Eq("Azure, software engineering/architecture, Scrum, SharePoint, VSTS/TFS, .NET and other funny things",
            feed.Description);
    ...
    {
        var feed = (Rss20Feed)(await Reader.ReadFromFileAsync("Feeds/Rss20CodeHollowCom.xml").ConfigureAwait(false))
            .SpecificFeed;
    ...
    {
        var feed = (Rss20Feed)(await Reader.ReadFromFileAsync("Feeds/Rss20ContentWindCom.xml").ConfigureAwait(false))
            .SpecificFeed;
        Eq("ContentWind", feed.Title);
        Eq("http://content-wind.com", feed.Link);
        Eq("Do, 22 Dez 2016 17:36:00 +0000", feed.LastBuildDateString);
    ...
    public void TestRss20ParseMoscowTimes()
    {
        ...
        Eq(
            "https://themoscowtimes.com/articles/russian-state-tv-praises-trump-for-avoiding-democracy-in-inauguration-speech-56901",
            item.Link);
        Eq(
            "Though he welcomed the end of Obama’s presidency as the start of a bright new era, the Kremlin’s “chief propagandist” quickly found himself struggling to find convincing scapegoats for the world’s problems this week.",
            item.Description);
    ...
    {
        var feed = (Rss20Feed)(await Reader.ReadFromFileAsync("Feeds/Rss20MoscowTimes.xml").ConfigureAwait(false))
            .SpecificFeed;
        Eq("The Moscow Times - News, Business, Culture & Multimedia from Russia", feed.Title);
        Eq("https://themoscowtimes.com/", feed.Link);
        Eq(
    ...
    public void TestRss20ParseSwedishFeedWithIso8859_1()
    {
        ...
        Eq(
            "https://www.retriever-info.com/go/?a=30338&d=00201120180819281555686&p=200108&s=2011&sa=2016177&u=http%3A%2F%2Fwww.hoglandsnytt.se%2Fsvart-manad-dodsolycka-i-vetlanda%2F&x=33d88e677ce6481d9882de22c76e4234",
            item.Link);
        Eq(
            "Under juli 2018 omkom 39 personer och 1 521 skadades i vägtrafiken. Det visar de preliminära uppgifter som inkommit till Transportstyrelsen fram till den 15 augusti 2018. Det är åtta fler omkomna jämfört med juli månad 2017.",
            item.Description);
    ...
    {
        var feed =
            (Rss20Feed)(await Reader.ReadFromFileAsync("Feeds/Rss20ISO88591Intranet30.xml").ConfigureAwait(false))
            .SpecificFeed;
        Eq("intranet30", feed.Title);
        Eq("http://www.retriever-info.com", feed.Link);
        Eq("RSS 2.0 News feed from Retriever Norge AS", feed.Description);
    ...
    {
        var feed = (Rss20Feed)(await Reader.ReadFromFileAsync("Feeds/Rss20FeedWebResourceSample.xml")
            .ConfigureAwait(false)).SpecificFeed;
    ...
    [TestMethod]
    public void TestUndeclaredEntitiesMattPaynePhoto()
    {
        //This feed has an Undeclared Entity
        var feed = Reader.ReadFromFile("Feeds/UndeclaredEntityMattPaynePhoto.xml");

        Eq(feed.Items.Count, 12);
    }
}