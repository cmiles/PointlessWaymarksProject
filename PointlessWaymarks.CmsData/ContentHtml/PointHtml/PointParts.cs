    ...
    public static class PointParts
    {
        ...
        public static HtmlTag PointTextInfoDiv(PointContentDto point)
        {
            ...
            var pTag = new HtmlTag("p")
                .Text(
                    $"Lat: {Math.Round(point.Latitude, 5)}, Long: {Math.Round(point.Longitude, 5)}{(point.Elevation == null ? string.Empty : $", Elevation: {Math.Round(point.Elevation.MetersToFeet(), 0)}', {point.Elevation:F2}m")}, {CalTopoMapsLatLongLink(point)}, {GoogleMapsLatLongLink(point)}, {OsmCycleMapLatLongLink(point)}")
