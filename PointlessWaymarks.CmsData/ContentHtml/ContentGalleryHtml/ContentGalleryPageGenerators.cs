...
namespace PointlessWaymarks.CmsData.ContentHtml.ContentGalleryHtml
{
    ...
    {
        public static async Task<ContentGalleryPage> ContentGallery(DateTime? generationVersion,
            IProgress<string>? progress = null)
        {
            ...
            {
                //!!Content Type List!!
                var fileContent = await db.FileContents.Where(x => !x.IsDraft).Select(x => x.CreatedOn.Date)
                    .ToListAsync();
                var geoJsonContent = await db.GeoJsonContents.Where(x => !x.IsDraft).Select(x => x.CreatedOn.Date)
                    .ToListAsync();
                var imageContent = await db.ImageContents.Where(x => !x.IsDraft && x.ShowInSearch)
                    .Select(x => x.CreatedOn.Date).ToListAsync();
                var lineContent = await db.LineContents.Where(x => !x.IsDraft).Select(x => x.CreatedOn.Date)
                    .ToListAsync();
                var noteContent = await db.NoteContents.Where(x => !x.IsDraft).Select(x => x.CreatedOn.Date)
                    .ToListAsync();
                var photoContent = await db.PhotoContents.Where(x => !x.IsDraft).Select(x => x.CreatedOn.Date)
                    .ToListAsync();
                var pointContent = await db.PointContents.Where(x => !x.IsDraft).Select(x => x.CreatedOn.Date)
                    .ToListAsync();
                var postContent = await db.PostContents.Where(x => !x.IsDraft).Select(x => x.CreatedOn.Date)
                    .ToListAsync();
                var videoContent = await db.VideoContents.Where(x => !x.IsDraft).Select(x => x.CreatedOn.Date)
                    .ToListAsync();
            ...
            if (allDates.Count == 0)
                return new ContentGalleryPage
                {
                    ...
                    PageUrl = UserSettingsSingleton.CurrentSettings().ContentGalleryUrl(),
            ...
            for (var i = 0; i < allDates.Count; i++)
            {
                ...
                var dateContent = await Db.ContentCreatedOnDay(loopDate.ToDateTime(TimeOnly.MinValue));
                dateContent.Reverse();
        ...
        public static HtmlTag ContentCardFromContentCommon(IContentCommon content)
        {
            ...
            if (!string.IsNullOrWhiteSpace(content.Summary) &&
                !(content.Summary.Equals(content.Title, StringComparison.OrdinalIgnoreCase) || content.Summary![..^1]
                    .Equals(content.Title, StringComparison.OrdinalIgnoreCase) || content.Title!.Contains(content.Summary, StringComparison.InvariantCulture) || content.Title.Contains(content.Summary![..^1], StringComparison.InvariantCulture)))
            ...
            summaryLines.ForEach(x => compactContentMainTextContentDiv.Children.Add(new DivTag().Text(x).AddClass("cg-card-text")));
            ...
            footerDiv.Children.Add(new DivTag().AddClass("cg-card-footer-date").Attr("style","display: none;").Text(content.CreatedOn.ToString("M/d/yyyy")));
