...
namespace PointlessWaymarks.CmsData.ExcelImport
{
    ...
    ImportRow toProcess, string columnName)
        {
            ...
            var stringValue = toProcess.Values[contentIdColumn.ExcelSheetColumn];
        ...
        public static ExcelValueParse<double?> GetDoubleFromExcelRow(ExcelHeaderRow headerInfo, ImportRow toProcess,
            string columnName)
        {
            var contentIdColumn = headerInfo.Columns.Single(x => string.Equals(x.ColumnHeader,
                columnName.TrimNullToEmpty(), StringComparison.CurrentCultureIgnoreCase));
        ...
        public static ExcelValueParse<bool?> GetBoolFromExcelRow(ExcelHeaderRow headerInfo, ImportRow toProcess,
            string columnName)
        {
            var contentIdColumn = headerInfo.Columns.Single(x => string.Equals(x.ColumnHeader,
                columnName.TrimNullToEmpty(), StringComparison.CurrentCultureIgnoreCase));
        ...
        public static ExcelValueParse<Guid?> GetGuidFromExcelRow(ExcelHeaderRow headerInfo, ImportRow toProcess,
            string columnName)
        {
            var contentIdColumn = headerInfo.Columns.SingleOrDefault(x => string.Equals(x.ColumnHeader,
                columnName.TrimNullToEmpty(), StringComparison.CurrentCultureIgnoreCase));
        ...
        public static ExcelValueParse<int?> GetIntFromExcelRow(ExcelHeaderRow headerInfo, ImportRow toProcess,
            string columnName)
        {
            var contentIdColumn = headerInfo.Columns.Single(x => string.Equals(x.ColumnHeader,
                columnName.TrimNullToEmpty(), StringComparison.CurrentCultureIgnoreCase));
     ...
     ImportRow toProcess)
        {
            ...
            {
                var stringValue = toProcess.Values[loopColumns.ExcelSheetColumn];
        ...
        public static ExcelValueParse<string> GetStringFromExcelRow(ExcelHeaderRow headerInfo, ImportRow toProcess,
            string columnName)
        {
            var contentIdColumn = headerInfo.Columns.Single(x => string.Equals(x.ColumnHeader,
                columnName.TrimNullToEmpty(), StringComparison.CurrentCultureIgnoreCase));
        ...
        public static async Task<(bool hasError, string errorNotes, dynamic? processContent)> ImportContentFromExcelRow(
            ExcelHeaderRow headerInfo, ImportRow toProcess)
        {
            // ReSharper disable once StringLiteralTypo
            var contentId = GetGuidFromExcelRow(headerInfo, toProcess, "contentid");
        ...
        public record ImportRow(List<string> Values, string RowIdentifier);

        public static async Task<ExcelContentTableImportResults> ImportContentTable(List<ImportRow>? toProcess,
    IProgress<string>? progress = null)
        {
            ...
            var headerInfo = new ExcelHeaderRow(toProcess.First().Values);
        ...
        public static async Task<ExcelContentTableImportResults> ImportFromTopMostExcelInstance(IProgress<string>? progress = null)
        {
            progress?.Report($"Getting Reference to Excel");

            var currentExcel = PointlessWaymarks.ExcelInteropExtensions.Session.Current.TopMost;

            if (currentExcel?.ActiveWorkbook?.ActiveSheet == null)
            {
                progress?.Report("No Active Excel Instance with an open File Found?");
                return await ImportExcelContentTable(null, progress);
            }
            ...
            if (tableRange == null)
            {
                progress?.Report($"No data found in Workbook {currentExcel.ActiveWorkbook.Name} - Worksheet {worksheet.Name}?");
                return await ImportExcelContentTable(null, progress);
            }

            if (tableRange.Cells.Count < 2 || tableRange.Rows.Count < 2)
            {
                progress?.Report($"Not enough data found in Workbook {currentExcel.ActiveWorkbook.Name} - Worksheet {worksheet.Name}?");
                return await ImportExcelContentTable(null, progress);
            }

            progress?.Report($"Excel Open File Import - Workbook {currentExcel.ActiveWorkbook.Name} - Worksheet {worksheet.Name} - Range {tableRange.Address}");
            ...
            var excelColumn = ((Range)tableRange.Cells[1, 1]).Column;

            var translated = new List<ImportRow>();
            ...
            for (int r = 1; r <= rowLength; r++)
            {
                ...
                {
                    valuesToAdd.Add(excelObjects[r,c].ToString() ?? string.Empty);
                }

                translated.Add(new ImportRow(valuesToAdd, $"Row {excelRow}, Column {ExcelUtilities.ExcelColumnFromNumber(excelColumn)}"));
