using PointlessWaymarks.FeedReader.Parser;
...
public static class Reader
{
    ...
    public static HtmlFeedLink GetAbsoluteFeedUrl(string pageUrl, HtmlFeedLink feedLink)
    {
        ...
        if (string.IsNullOrWhiteSpace(tmpUrl)) throw new UrlNotFoundException("Blank Url Submitted?");
    ...
    {
        return (await GetFeedUrlsFromUrlAsync(url, cancellationToken).ConfigureAwait(false)).Select(x => x.Url)
            .ToArray();
    }
