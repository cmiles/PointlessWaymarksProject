...
$publishPath = "M:\PointlessWaymarksPublications\PointlessWaymarks.CmsContentEditor"
if(!(test-path -PathType container $publishPath)) { New-Item -ItemType Directory -Path $publishPath }

& $msBuild .\PointlessWaymarks.CmsContentEditor\PointlessWaymarks.CmsContentEditor.csproj -t:publish -p:PublishProfile=.\PointlessWaymarks.CmsContentEditor\Properties\PublishProfile\FolderProfile.pubxml -verbosity:minimal
