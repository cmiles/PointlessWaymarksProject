...
EF Lazy Loading Experiment Failure: the original Entity Framework dealt with Lazy Loading in a way that either matched, or shaped, my expectations about how a Lazy Loading ORM should work. EF6 is basically dead at this point and it makes sense to move to EF Core. But EF Core doesn't have lazy loading support out of the box. To some extent I understand that decision both because of EF Core having a much wider variety of data sources in mind and because you certainly can hurt performance with EF6's lazy loading. But EF Core does have a package to add Lazy Loading and for the Cloud Backup program I decided to return to traditional db relationship modeling and Lazy Loading (the CMS project uses SQLite but it made sense at the time - and actually still makes sense to me - to design the db with flat json documents in mind and tables without relationships - more of a document db spirit).

I recently moved some Backups to Cloudflare R2 and in doing that and re-running some backups from scratch I really noticed how absurdly poor the Cloud Backup performance was. To some extent this was by design in the sense that I wanted it to run in the background without pinning CPU/Memory/Disk by spinning up a huge number of parallel threads - but at the same I didn't mean for it to be this bad! So I started tracking down problems and basically found:
 - Lazy Loading just wasn't helping - I removed it and let Resharper help with a number of the Includes
 - There were a number of opportunities for Parallel.ForEach style code
 - SQLite insert performance was a huge issue. I still suspect that there is a configuration or other EF Core detail I am missing but even after trying to follow the common perf advice online the performance was just absurdly bad. I also had some mystery issues with a package that adds bulk inserts to EF so in the end I decided to just write old school commands to do the inserts. In the past this would have been very annoying but I have to say that Copilot did a nice job of generating this code - although as seems to be the typical case these days it did leave me some gotchas. With a little troubleshooting the Command style inserts were so much faster that at first I suspected it wasn't actually doing the insert!!! This approach was a win but overall I'm disappointed to drop away from EF Core to accomplish this - multiple database approaches results in more mainenance pain.

Cloudflare R2: Overall the move to support Cloudflare R2 has been a win - the pricing means that one more source of potentially bad/big bills is eliminated and the performance seems great! But I've hit API surprises much sooner than expected - in fairness I guess you could call this my fault as there is a detailed list at https://developers.cloudflare.com/r2/api/s3/api/ - but this little 'Copy' error gem was quite painful - https://stackoverflow.com/questions/77633198/why-doesnt-copyobject-for-cloudflare-r2-not-work-with-the-aws-sdk-for-net - with what I had considered a fairly simplistic use of the S3 API I was surprised that I ran into problems so quickly. I think this was a great reminder that 1) You have to be really careful with 'compatible APIs' (99.9% compatible is not helpful if the .1% is something you rely on!) and 2) Testing against anything but the real service is a risk - I have tests setup against a Adobe S3Mock which would never have helped me catch this copy problem.
