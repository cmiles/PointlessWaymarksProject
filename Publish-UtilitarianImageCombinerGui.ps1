...
$publishPath = "M:\PointlessWaymarksPublications\PointlessWaymarks.UtilitarianImageCombinerGui"
...
& $msBuild .\PointlessWaymarks.UtilitarianImageCombinerGui\PointlessWaymarks.UtilitarianImageCombinerGui.csproj -t:publish -p:PublishProfile=.\PointlessWaymarks.UtilitarianImageCombinerGui\Properties\PublishProfile\FolderProfile.pubxml -verbosity:minimal

if ($lastexitcode -ne 0) { throw ("Exec: " + $errorMessage) }

& 'C:\Program Files (x86)\Inno Setup 6\ISCC.exe' .\Publish-InnoSetupInstaller-UtilitarianImageCombinerGui.iss /DVersion=$PublishVersion /DGitCommit=$GitCommit
