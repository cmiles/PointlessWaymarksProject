...
public partial class FilesWrittenLogListContext
{
        ...
        bool loadInBackground)
    {
        ...
        return new FilesWrittenLogListContext(factoryStatusContext, loadInBackground, factoryGenerationChoices,
            factoryItems);
    }
    ...
    public async Task SiteDeletedFilesReport()
    {
        await ThreadSwitcher.ResumeBackgroundAsync();
        var results =
            await S3GeneratedSiteComparisonForDeletions.RunReport(S3CmsTools.S3AccountInformationFromSettings(),
        ...
        var newUploadWindow = await S3DeletionsWindow.CreateInstance(S3CmsTools.S3AccountInformationFromSettings(),
            results
    ...
    [BlockingCommand]
    public async Task SiteMissingAndChangedFilesByHashReport()
    {
        await ThreadSwitcher.ResumeBackgroundAsync();
        var results =
            await S3GeneratedSiteComparisonWithHashesForAdditionsAndChanges.RunReport(
                S3CmsTools.S3AccountInformationFromSettings(), StatusContext.ProgressTracker());

        if (results.ErrorMessages.Any())
        {
            if (results.FileHashMismatches.Any() || results.MissingFiles.Any())
            {
                var cancelOrContinue = await StatusContext.ShowMessage("Missing And Changed Files Report Error",
                    $"The report returned {results.FileHashMismatches.Count + results.MissingFiles.Count} results and the errors below - Cancel or Continue?{Environment.NewLine}{Environment.NewLine}{string.Join($"{{Environment.NewLine}}{Environment.NewLine}", results.ErrorMessages)}",
                    ["Cancel", "Continue"]);
        ...
        var toUpload = results.FileHashMismatches.Concat(results.MissingFiles).ToList();
        ...
        var newUploadWindow = await S3UploadsWindow.CreateInstance(S3CmsTools.S3AccountInformationFromSettings(),
            toUpload,
