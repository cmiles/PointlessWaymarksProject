    ...
    ICheckForChangesAndValidation, IDropTarget, IWebViewMessenger
{
    ...
    {
        HasChanges = PropertyScanners.ChildPropertiesHaveChanges(this) || MapElementsListHasChanges();
        ...
        bool guiNotificationAndMapRefreshWhenAdded = false)
    {
        ...
        if (guiNotificationAndMapRefreshWhenAdded)
            await StatusContext.ToastSuccess($"Added File - {possibleFile.Title}");
    }
        ...
        bool guiNotificationAndMapRefreshWhenAdded = false)
    {
        ...
        if (guiNotificationAndMapRefreshWhenAdded)
            await StatusContext.ToastSuccess($"Added Point - {possibleGeoJson.Title}");
    }
        ...
        bool guiNotificationAndMapRefreshWhenAdded = false)
    {
        ...
        if (guiNotificationAndMapRefreshWhenAdded)
            await StatusContext.ToastSuccess($"Added Image - {possibleImage.Title}");
    }
        ...
        bool guiNotificationAndMapRefreshWhenAdded = false)
    {
        ...
        if (guiNotificationAndMapRefreshWhenAdded)
            await StatusContext.ToastSuccess($"Added Point - {possibleLine.Title}");
    }
        ...
        bool guiNotificationAndMapRefreshWhenAdded = false)
    {
        ...
        if (guiNotificationAndMapRefreshWhenAdded)
            await StatusContext.ToastSuccess($"Added Photo - {possiblePhoto.Title}");
    }
        ...
        bool guiNotificationAndMapRefreshWhenAdded = false)
    {
        ...
        if (guiNotificationAndMapRefreshWhenAdded)
            await StatusContext.ToastSuccess($"Added Point - {possiblePoint.Title}");
    }
        ...
        bool guiNotificationAndMapRefreshWhenAdded = false)
    {
        ...
        if (guiNotificationAndMapRefreshWhenAdded)
            await StatusContext.ToastSuccess($"Added Post - {possiblePost.Title}");
    }
        ...
        bool guiNotificationAndMapRefreshWhenAdded = false)
    {
        ...
        if (guiNotificationAndMapRefreshWhenAdded)
            await StatusContext.ToastSuccess($"Added Video - {possibleVideo.Title}");
    }
    ...
    public bool MapElementsListHasChanges()
    {
        if (MapElements.Any(x => x.ContentId() is null)) return true;

        var mapElementGuids = MapElements.Select(x => x.ContentId()!.Value).OrderBy(x => x).ToList();
        var dbElementGuids = DbElements.Select(x => x.ElementContentId).OrderBy(x => x).ToList();

        return !mapElementGuids.SequenceEqual(dbElementGuids);
    }
    ...
    private void MapElementsOnCollectionChanged(object? sender, NotifyCollectionChangedEventArgs e)
    {
        ...
        if (!SuspendMapRefresh) StatusContext.RunFireAndForgetNonBlockingTask(() => RefreshMapPreview(MapBounds));
    }
