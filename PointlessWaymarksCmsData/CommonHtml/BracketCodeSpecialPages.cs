    ...
    public static class BracketCodeSpecialPages
    {
        public static string SpecialPagesCodeProcess(string toProcess, IProgress<string> progress)
        {
            ...
            var specialPageLookup = new List<(string bracketCode, string defaultDisplayString, string url)>
            {
                ...
                ("indexrss", "RSS", $"https:{UserSettingsSingleton.CurrentSettings().RssIndexFeedUrl()}"),
                ("filerss", "RSS", $"https:{UserSettingsSingleton.CurrentSettings().FileRssUrl()}"),
                ("imagerss", "RSS", $"https:{UserSettingsSingleton.CurrentSettings().ImageRssUrl()}"),
                ("linkrss", "RSS", $"https:{UserSettingsSingleton.CurrentSettings().LinkRssUrl()}"),
                ("noterss", "RSS", $"https:{UserSettingsSingleton.CurrentSettings().NoteRssUrl()}"),
                ("photorss", "RSS", $"https:{UserSettingsSingleton.CurrentSettings().PhotoRssUrl()}"),
